...
heli( [], AccumulatedDistance, [CurrentPoint|RouteSoFar], [] ):-helicopterBasePoint(BasePoint), distanceBetweenTwoPoints(CurrentPoint,BasePoint, Dist), 
append([BasePoint], [CurrentPoint|RouteSoFar], RouteALL), NewDistance is AccumulatedDistance + Dist, storeRouteIfBetter(NewDistance, RouteALL),fail.


% Pick up another passenger:
heli( PassengersToPickUp, AccumulatedDistance, [CurrentPoint|RouteSoFar], DestinationsOfPassengersInHelicopter ):- select([From,Dest], PassengersToPickUp, R), length(DestinationsOfPassengersInHelicopter,N), N < 2, distanceBetweenTwoPoints(CurrentPoint, From, Dist), NewDistance is AccumulatedDistance + Dist, append([From],[CurrentPoint|RouteSoFar], RouteALL),  append([Dest],DestinationsOfPassengersInHelicopter, Dests), heli(R, NewDistance, RouteALL, Dests),fail.


% Drop off one of the passengers in the helicopter:
heli( PassengersToPickUp, AccumulatedDistance, [CurrentPoint|RouteSoFar], DestinationsOfPassengersInHelicopter):- select(Destination, DestinationsOfPassengersInHelicopter,R), distanceBetweenTwoPoints(CurrentPoint,Destination, Dist), NewDistance is AccumulatedDistance + Dist, append([Destination],[CurrentPoint|RouteSoFar], RouteALL), heli(PassengersToPickUp, NewDistance, RouteALL,R),fail.
