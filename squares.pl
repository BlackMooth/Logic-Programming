...
insideBigSquare(0, _, [], []).
insideBigSquare(N, Big, [X|Sides], [V|Vars]):- Max is Big-X+1, V in 1..Max, 
											   N2 is N-1, insideBigSquare(N2, Big, Sides, Vars).
main:- 
    ejemplo(3,Big,Sides),
...
nonoverlapping(0, [], [], []).
nonoverlapping(_, [X|Sides], [R|Rowvars], [C|Colvars]):- nonoverlapping2(X, Sides, R, Rowvars, C, Colvars),
		nonoverlapping(_, Sides, Rowvars, Colvars).

nonoverlapping2(_, [], _, [], _, []).
nonoverlapping2(X, [X2|Sides], R, [R2|RowVars], C, [C2|ColVars]):-
		nonoverlapping3(X, X2, R, R2, C, C2),
