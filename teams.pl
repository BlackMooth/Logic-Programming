...
complexWorkers(W1,W2):-workerScore(W1,S1), workerScore(W2,S2), S is S1+S2, complexThreshold(Threshold), S >= Threshold.

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% 1.- Declare SAT variables to be used
% wt(W,T) means "worker W is in team T"
satVariable( wt(W,T) ):- worker(W), team(T).
% co(T) means "T is a complex team"
satVariable( co(T) ):- team(T).
...
writeClauses(MaxComplexTeams):-
	atMostComplexTeams(MaxComplexTeams),
	eachWorkerHasExactlyOneTeam,
	teamComplexImpliesComplex,
	eachTeamAtMostSize,
	eachTeamAtLestSize,
	workersCompatible,
    true,!.
...
teamComplexImpliesComplex:- team(T), complexWorkers(W1,W2), W1 \= W2, writeClause([-wt(W1,T), -wt(W2,T), co(T)]), fail.
teamComplexImpliesComplex.

atMostComplexTeams(MaxComplexTeams):- findall(co(T), team(T), Lits), atMost(MaxComplexTeams, Lits), fail.
atMostComplexTeams(_).
...
eachTeamAtLestSize:-team(T), minSize(M), findall(wt(W,T), worker(W), Lits), atLeast(M,Lits), fail.
eachTeamAtLestSize.

eachWorkerHasExactlyOneTeam:-worker(W), findall(wt(W,T), team(T), Lits), exactly(1,Lits), fail.
eachWorkerHasExactlyOneTeam.
...
costOfThisSolution(M,Cost):- findall(T,member(co(T),M), Lits), length(Lits,Cost),! .
