...
writeClauses(MaxNumProf):-
	exactlyOneValidRoomPerCourse,
	exactlyOneValidProfessorPerCourse,
	eachCourseAtMostOneHourPerDay,
	cdhimpliescd,
	cdimpliescdh,
	exactlyCourseHours,
	atMostOneCoursePerDHR,
	crANDcdhTOcdhr,
	cdhrTOcrANDcdh,	
	cdhANDcpTOCDHP,
	cdhpTOcdhANDcp,
...
atMostOneCoursePerDHR:- hour(H), room(R), day(D), findall(cdhr(C,D,H,R), course(C), Lits), atMost(1,Lits), fail.
atMostOneCoursePerDHR.
...
cdhANDcpTOCDHP:- course(C), day(D), professor(P), courseProfessors(C,PS), member(P,PS),hour(H), writeClause([-cdh(C,D,H), -cp(C,P), cdhp(C,D,H,P)]), fail.
cdhANDcpTOCDHP.


% CDHP -> CDH AND CP
cdhpTOcdhANDcp:- course(C), day(D), hour(H), courseProfessors(C,PS), member(P,PS), writeClause([-cdhp(C,D,H,P), cdh(C,D,H)]), writeClause([-cdhp(C,D,H,P), cp(C,P)]), fail.
cdhpTOcdhANDcp. 
...
exactlyOneValidRoomPerCourse:- course(C), courseRooms(C,LR), findall(cr(C,R),member(R,LR), Lits), exactly(1,Lits),fail.
exactlyOneValidRoomPerCourse:- course(C), courseRooms(C,LR), room(R), \+member(R,LR),  writeClause([-cr(C,R)]),fail. 
exactlyOneValidRoomPerCourse.

% Exactly 1 Valid Professor per each Course comprovar si fa falta lo de treure les unavailable
exactlyOneValidProfessorPerCourse:- course(C), courseProfessors(C,PR), findall(cp(C,P), member(P,PR), Lits), exactly(1,Lits), fail.
exactlyOneValidProfessorPerCourse:- course(C), courseProfessors(C,PR), professor(P), \+member(P,PR), writeClause([-cp(C,P)]),fail. 
exactlyOneValidProfessorPerCourse.
...
pTOcp:-professor(P),  findall(cp(C,P),(courseProfessors(C,PS),member(P,PS)), L) , writeClause([-p(P)|L])  ,fail. 
pTOcp.
cpTOp:- course(C), courseProfessors(C,PS),member(P,PS), writeClause([-cp(C,P), p(P)]) ,fail.
cpTOp.
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
